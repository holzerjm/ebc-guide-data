/* Red Hat EBC visitor guides — data.js validation core.
...
(function (root, factory) {
  ...
  /* ---------- the city registry ----------
     Every per-city fact lives here and nowhere else. This file is copied verbatim into each guide repo, so
     "the validator knows about Raleigh" is: add a row, re-copy the file. It sits under CODEOWNERS on
     purpose — a pull request must not be able to widen a bounding box and pass its own check. */
  var CITIES = {
    boston: {
      key: "boston", label: "Boston", area: "the Boston area",
      state: "Massachusetts", stateAbbr: "MA",
      bbox: { latMin: 42.20, latMax: 42.45, lngMin: -71.25, lngMax: -70.90 },
      canon: ["dining", "activities", "hotels", "travel"],
      dataPath: "boston/data.json",
      rawUrl: "https://raw.githubusercontent.com/holzerjm/ebc-guide-data/main/boston/data.json",
      guideUrl: "https://people.redhat.com/jholzer/EBC/BostonVisitorGuide/"
    },
    raleigh: {
      key: "raleigh", label: "Raleigh", area: "the Raleigh area",
      state: "North Carolina", stateAbbr: "NC",
      bbox: { latMin: 35.65, latMax: 35.95, lngMin: -78.95, lngMax: -78.45 },
      canon: ["dining", "bars", "hotels", "activities", "travel"],
      dataPath: "raleigh/data.json",
      rawUrl: "https://raw.githubusercontent.com/holzerjm/ebc-guide-data/main/raleigh/data.json",
      guideUrl: "https://people.redhat.com/jholzer/EBC/RaleighVisitorGuide/"
    }
  };
  function cityKeys() { return Object.keys(CITIES); }
  /* hasOwnProperty, not CITIES[k]: cityFor is called with a string taken off an issue label. */
  function cityFor(k) { return (k && Object.prototype.hasOwnProperty.call(CITIES, k)) ? CITIES[k] : null; }
  /* "boston/data.json", "./raleigh/data.json", "/tmp/x/boston/data.json" -> that row, else null.
     Pure string work, no node:path, so the browser copy still loads as a plain <script>. */
  function cityForPath(p) {
    var m = /(?:^|[\\/])([a-z][a-z0-9-]*)[\\/]data\.json$/.exec(String(p || "").replace(/\\/g, "/"));
    return m ? cityFor(m[1]) : null;
  }

  /* The city is asserted twice and the two must agree: `declared` is where the file LIVES (its directory in
     CI, the city: label on an issue, the one-line CITY in each guide's validate.js); meta.city is what the
     file SAYS. `declared` is OPTIONAL so every existing one-argument caller keeps working — when it is
     absent the file speaks for itself, which is right for a data.js a colleague pasted into validate.html. */
  function resolveCity(D, declared) {
    var inFile = (D && D.meta && typeof D.meta.city === "string") ? D.meta.city : "";
    if (declared && !cityFor(declared))
      return { error: 'Unknown city "' + declared + '". Known: ' + cityKeys().join(", ") + "." };
    if (inFile && !cityFor(inFile))
      return { error: 'meta.city "' + inFile + '" is not a city this validator knows (' + cityKeys().join(", ") + ")." };
    if (declared && inFile && declared !== inFile)
      return { error: 'This file says meta.city "' + inFile + '" but it is being checked as "' + declared +
        '" — one of the two is wrong. The usual cause is an entry drafted into the wrong city\'s file.' };
    var key = declared || inFile;
    if (!key) return { error: 'No city given and the file has no meta.city. Add "city": "<' + cityKeys().join("|") + '>" to meta.' };
    return { city: cityFor(key), missingMeta: !inFile };
  }
  ...
  /* ---------- validation ---------- */
  function validate(D, declaredCity) {
    ...
    /* Resolve the city first and stop on failure: with the wrong box, 85 good venues produce 85 errors and
       bury the one that matters. */
    var rc = resolveCity(D, declaredCity);
    if (rc.error) { err("meta.city", rc.error); return { errors: errors, warnings: warnings, stats: {} }; }
    var CITY = rc.city, BBOX = CITY.bbox;
    if (rc.missingMeta) warn("meta", 'No meta.city — add "city": "' + CITY.key + '" so the file says which guide it belongs to.');
    ...
    CITY.canon.forEach(function (k) { if (!D.sections[k]) warn("sections", 'expected section "' + k + '" is missing — its tab would be empty.'); });
    Object.keys(D.sections).forEach(function (sk) {
      if (CITY.canon.indexOf(sk) === -1) warn("sections", 'section "' + sk + '" is not one of ' + CITY.label + "'s sections, so the guide has no tab for it.");
    });
    ...
    Object.keys(D.sections).forEach(function (sk) {
      ...
      sec.items.forEach(function (it, idx) {
        ...
        else if (it.lat < BBOX.latMin || it.lat > BBOX.latMax || it.lng < BBOX.lngMin || it.lng > BBOX.lngMax) err(W2, "coordinates (" + it.lat + ", " + it.lng + ") are outside " + CITY.area + " — they look wrong.");
  ...
  return { parseDataJs: parseDataJs, validate: validate, diff: diff, checkVersion: checkVersion,
           cities: CITIES, cityKeys: cityKeys, cityFor: cityFor, cityForPath: cityForPath, resolveCity: resolveCity };
});
